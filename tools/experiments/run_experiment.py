...
class RunnerConfig:
    ...
    prometheus_resource_settle_seconds: float = 30.0
...
def parse_args(argv: Sequence[str] | None = None) -> RunnerConfig:
    ...
    parser.add_argument("--prometheus-resource-settle-seconds", type=non_negative_float, default=float(os.getenv("LIVEEDGECAST_PROMETHEUS_RESOURCE_SETTLE_SECONDS", "30")), help="Seconds to wait after the experiment before collecting cAdvisor resource range queries and to extend only those resource query windows. This reduces intermittent missing worker CPU samples caused by scrape alignment. Default: 30.")
    ...
    return RunnerConfig(
        ...
        prometheus_resource_settle_seconds=args.prometheus_resource_settle_seconds,
...
def prometheus_query(config: RunnerConfig, query: str, start: float, end: float, step: int = 5, controller_label_selector: str | None = None) -> dict[str, Any]:
    ...
    try:
        with urlopen(Request(url, headers={"User-Agent": "liveedgecast-experiment/1"}), timeout=30) as response:
            payload = json.loads(response.read().decode("utf-8"))
        return {"available": True, "query": rendered_query, "template_query": query, "rendered_query": rendered_query, "range_start": start, "range_end": end, "response": payload}
    except Exception as exc:
        return {"available": False, "query": locals().get("rendered_query", query), "template_query": query, "rendered_query": locals().get("rendered_query", query), "range_start": start, "range_end": end, "error": {"type": type(exc).__name__, "message": str(exc)}}
...
def collect_prometheus(config: RunnerConfig, dirs: dict[str, Path], start: float, end: float, controller_label_selector: str | None = None) -> dict[str, Any]:
    ...
    resource_metric_names = {"pod_cpu_rate", "pod_memory_working_set"}
    resource_query_end = end
    if resource_settle_seconds > 0:
        time.sleep(resource_settle_seconds)
        resource_query_end = max(end, now_epoch())
    results = {
        "_metadata": {
            ...
            "started_at": start,
            "ended_at": end,
            "resource_query_ended_at": resource_query_end,
            "resource_settle_seconds": resource_settle_seconds,
    ...
    for name, query in DEFAULT_PROMQL.items():
        query_end = resource_query_end if name in resource_metric_names else end
        results[name] = prometheus_query(config, query, start, query_end, controller_label_selector=controller_label_selector)
