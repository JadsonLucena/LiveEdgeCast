...
- Métricas de recurso do cAdvisor/kubelet, especialmente `pod_cpu_rate` de workers curtos, dependem do alinhamento entre vida do Pod, scrape interval e a janela de `rate(...[1m])`. O runner aguarda `--prometheus-resource-settle-seconds` (ou `LIVEEDGECAST_PROMETHEUS_RESOURCE_SETTLE_SECONDS`, padrão `30`) e estende somente as consultas de CPU/memória para reduzir campanhas intermitentes sem alterar a janela usada por Pod-seconds. O runner consulta tanto labels atuais (`pod`/`container`) quanto legados (`pod_name`/`container_name`); quando as recording rules `liveedgecast:component:cpu_cores:rate5m` e `liveedgecast:component:memory_working_set_bytes` existem, elas são usadas como fallback por componente para preencher workers ausentes na consulta bruta por Pod.
