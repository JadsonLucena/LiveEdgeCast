...
| `worker_pods_available` | Gauge | `namespace` | pods | Muito baixa: um namespace por ambiente. | Número de Pods de worker com condição `Ready=True`; a implementação é atualizada periodicamente pelo controller e não subtrai workers já associados a streams. | Readiness/capacidade bruta observada; não usar como capacidade livre de alocação. |
...
| `container_network_receive_packets_total` | Counter | `namespace`, `pod`, `interface` e labels do alvo | pacotes | Média: interface/Pod conforme runtime. | Pacotes recebidos observados por cAdvisor/kubelet; filtre `interface!="lo"`. | Fonte primária para pacotes RX por Pod/componente. |
| `container_network_transmit_packets_total` | Counter | `namespace`, `pod`, `interface` e labels do alvo | pacotes | Média: interface/Pod conforme runtime. | Pacotes transmitidos observados por cAdvisor/kubelet; filtre `interface!="lo"`. | Fonte primária para pacotes TX por Pod/componente. |
| `container_network_receive_errors_total` | Counter | `namespace`, `pod`, `interface` e labels do alvo | erros | Média: interface/Pod conforme runtime. | Erros de recebimento observados por cAdvisor/kubelet; filtre `interface!="lo"`. | Diagnóstico de perdas/problemas de rede por Pod. |
| `container_network_transmit_errors_total` | Counter | `namespace`, `pod`, `interface` e labels do alvo | erros | Média: interface/Pod conforme runtime. | Erros de transmissão observados por cAdvisor/kubelet; filtre `interface!="lo"`. | Diagnóstico de perdas/problemas de rede por Pod. |
| `liveedgecast:pod:network_receive_bytes_per_second:rate5m` | Recording rule | `namespace`, `pod` | bytes/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa RX por Pod somada entre interfaces não-loopback. | Painéis por Pod e base para agregados por componente. |
| `liveedgecast:pod:network_transmit_bytes_per_second:rate5m` | Recording rule | `namespace`, `pod` | bytes/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa TX por Pod somada entre interfaces não-loopback. | Painéis por Pod e base para agregados por componente. |
| `liveedgecast:pod:network_receive_packets_per_second:rate5m` | Recording rule | `namespace`, `pod` | pacotes/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa de pacotes RX por Pod somada entre interfaces não-loopback. | Diagnóstico de volume de pacotes por Pod. |
| `liveedgecast:pod:network_transmit_packets_per_second:rate5m` | Recording rule | `namespace`, `pod` | pacotes/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa de pacotes TX por Pod somada entre interfaces não-loopback. | Diagnóstico de volume de pacotes por Pod. |
| `liveedgecast:pod:network_receive_errors_per_second:rate5m` | Recording rule | `namespace`, `pod` | erros/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa de erros RX por Pod somada entre interfaces não-loopback. | Alertas/triagem de problemas de rede por Pod. |
| `liveedgecast:pod:network_transmit_errors_per_second:rate5m` | Recording rule | `namespace`, `pod` | erros/s | Baixa: uma série por Pod LiveEdgeCast. | Taxa de erros TX por Pod somada entre interfaces não-loopback. | Alertas/triagem de problemas de rede por Pod. |
...
| `liveedgecast:component:network_receive_bytes_per_second:rate5m` | Recording rule | `component` | bytes/s | Baixa: `proxy-lb`, `proxy`, `worker`, `controller`. | Taxa RX agregada por componente a partir da recording rule por Pod de 5 minutos. | Painéis de custo/recurso suavizados por componente. |
| `liveedgecast:component:network_transmit_bytes_per_second:rate5m` | Recording rule | `component` | bytes/s | Baixa: `proxy-lb`, `proxy`, `worker`, `controller`. | Taxa TX agregada por componente a partir da recording rule por Pod de 5 minutos. | Painéis de custo/recurso suavizados por componente. |
