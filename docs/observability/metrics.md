...
| `stream_lifecycle_phase_observations_total` | Counter | `phase`, `status`, `reason` | eventos | Baixa: fases fixas × status/razões controladas. | Conta observações derivadas, inclusive fases pendentes ou descartadas por endpoint aproximado, timestamp ausente ou duração negativa. | Denominador de qualidade dos dados; reportar exclusões e limitações de medição. |
