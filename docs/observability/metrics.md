...
Controller and worker metrics include optional labels: `experiment_id`, `scenario`, `run_id`.
`streamKey` is intentionally not used as Prometheus label (only logs).

## Core controller counters
- stream_started_events_total{status,...}
- stream_ended_events_total{status,...}
- stream_active_total{...} (gauge)
- stream_allocation_active_total{...} (gauge)
...
## Proxy RTMP
- proxy_rtmp_active_streams{proxy_pod}
- proxy_rtmp_active_publishers{proxy_pod}
- proxy_rtmp_active_clients{proxy_pod}
- proxy_rtmp_stream_active{proxy_pod}
- proxy_rtmp_stats_scrape_errors_total{proxy_pod}
